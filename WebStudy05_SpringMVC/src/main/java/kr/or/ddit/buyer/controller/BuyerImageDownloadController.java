...
public class BuyerImageDownloadController {
	...
	public ResponseEntity<Resource> buyerImageDownload(
		...
		try {
			...
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,String.format("%s 제조사 없음.", buyerId));
		}
		...
		if(StringUtils.isBlank(filename)) {
			// 사업자 등록증 사본 없음.
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,String.format("%s 사업자 등록증 없음.", buyer.getBuyerName()));
		}
