...
<body>
	<h4>JSP 스펙의 기본 객체</h4>
	<pre>
		pageContext(Page Scope) : 가장 먼저 생성되는 기본 객체로 현재 jsp 페이지와 관련된 모든 정보(ex.기본객체)를 가진 객체
		...
		HttpServletRequest request(Request Scope) : http 방식에 따라 패키징된 요청에 대한 정보를 가진 객체
									요청이 전송되면 서버에 의해 생성되고, 그에 대한 응답이 전송되면 삭제됨(stateless특성에 따라서...). 
		...
		---> scope 라고 불리는 map 을 내부에 갖고 있는 객체들로,
			...
			setAttribute(String k, Object value), getAttribute(String k), removeAttribute(String k)
			getAttributeNames()
		...
		Throwable exception : 어플리케이션 내에서 발생한 모든 비정상 상황에 대한 정보를 가진 객체.
							에러 처리 목적으로 만들어진 jsp 에서만 활성화 됨.
