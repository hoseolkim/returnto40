...
<body>
	...
	<pre>
		세션
			시간(ConnectLsess) : 클라이언트가 web 어플리케이션을 사용하고 있는 기간
				최초의 요청이 발생하면 세션이 생성되고, 사용 종료 이벤트가 발생하면 세션이 제거됨.
				사용 종료 이벤트 종류
				1. log out
				2. timeout 이내에 새로운 요청이 발생하지 않으면, 종료로 간주함.
				3. session tracking mode 에 따라 세션 아이디가 재전송되지 않을 때. -> time out에 의해 쓰레기 세션이 사라짐
					ex) 클라이언트 측에서 쿠키를 제거함.
				4. 브라우저 종료 -> time out에 의해 쓰레기 세션이 사라짐
...
<!-- 			세션 하이잭에 취약하다 -->
		3. SSL (Secure Socket Layer) : 암호화 채널을 이용한 세션 아디이 공유(https)
			https 
	</pre>
</body>
</html>