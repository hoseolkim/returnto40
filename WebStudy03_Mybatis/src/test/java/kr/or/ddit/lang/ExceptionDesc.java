 ...
 *  	- checked exception(non RuntimeException) : 발생가능한 코드 블럭 내에서 어떤 형태로 해당 예외를 처리해야 하고,
 ...
 *  예외 처리 방법
 *   - 예외 회피(throws) : 메소드 콜스택에 저장된 복귀 주소에 따라 예외의 처리르 호출자에게 떠넘기는 전략.
 *   - 직접 처리 : try(Closable 객체 생성)~catch(exceptions..)~finally
 *   try{
 *   	// 잠재적 예외 발생 코드
 *   }catch(처리할 예외들 선언){
 *   	// 직접 처리
 *   	- 예외 전환 : 구체적인 상황을 명시할 수 있는 예외, 혹은 checked 예외를 unChecked 예외로 변경 - wrapping(원본 예외)
 *   			ex)
 *   			catch(IOException e){
 *   				throw new FileCopyFailureException(e);
 *   			}
 *   	- 예외 복구 : 예외발생시 일정시간 지연 후 해당 작업 재실행
 *   }finally{
